...
- 首頁「最新公告」卡片連到 `pages/news.html#post-x` 後，文章列表渲染完成會自動捲到指定文章，避免只停在最新文章頁頂端。
...
- 推文說明頁分享圖片區已加入 `完整文宣圖/完整文宣圖1.png` 到 `完整文宣圖13.png`，以及 `太陽神-1.png` 到 `太陽神-6.png`，共 19 張；使用折疊縮圖圖庫避免頁面過長，圖片檔已納入 Git。
