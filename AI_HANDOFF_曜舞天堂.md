...
- 首頁「最新公告」卡片使用 `pages/news.html#post-x` 連到最新文章頁；因文章列表由 JS 非同步載入，`assets/js/cms-posts.js` 內已補上渲染完成後依 hash 自動定位，避免只停在頁面頂端。
...
- 推文說明頁已新增包月推文代工三組優惠價 `$1000 TWD`，發文規定單日上限改為 60 顆（10 次）。
- 推文說明頁分享圖片區已加入 `完整文宣圖/完整文宣圖1.png` 到 `完整文宣圖13.png`，以及 `太陽神-1.png` 到 `太陽神-6.png` 的折疊縮圖圖庫；共 19 張，圖片檔已納入 Git。
- 直播推廣箱目前欄位是藍鑽、推文幣、直播獎勵兌換證；1/2/3 小時分別給 1/2/3 個兌換證。
- 變身圖鑑大全已加入變身合成機率：一般到高級 80%、高級到稀有 50%、稀有到英雄 35%、英雄到傳說 18%；「傳說到神話」目前不公開。
- 後台文章 Markdown 表格與一般內容頁 HTML 表格會自動包成可橫向滑動表格。手機若表格超出寬度，會顯示「左右滑動查看完整表格」提示。
- 活動公告目前已有永久 LINE 禮盒活動與兩篇開服活動：
