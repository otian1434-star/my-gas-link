...
## 2026-08-18 - 新增 08/18 更新歷程與七夕情人節活動
...
### 異動
- `data/posts.json` 新增 `0818-更新歷程`（狀態：預計更新），依原始編號逐項成區塊，涵蓋大陸地圖卡點回溯修正、巨蟻女皇金／銀翅膀強化加成、傲慢之塔傳送卷軸 NPC、支配之塔靈魂小怪掉落上調、暗黑武器製作 NPC、七夕情人節活動預告、遊戲輔助行動傳送按鈕優化、十二宮石板文字修正、法師武器蒐藏顯示修復，以及曜舞限定的吉爾塔斯 BOSS 強化與愛神／美神伺服器合併。
- `data/posts.json` 新增活動公告 `🎋 特別活動 ── 七夕情人節`（2026/08/19 ~ 2026/09/09，狀態：進行中、置頂），內容含特殊七夕贊助包、活動 NPC「特殊活動・轉世的牛郎」製作表、全 BOSS 掉落七種活動武器、（活動）武器與七夕千紙鶴的強化規則，以及七把永久武器的能力表。
- 胤天天堂（`yintian-forum`）同步新增這兩篇；依使用者指示，胤天版更新歷程不含吉爾塔斯 BOSS 強化與愛神／美神伺服器合併兩項。

### 驗證
- `node` 解析兩站 `data/posts.json` 通過：曜舞 19 筆、胤天 9 筆。
- 曜舞更新歷程 11 個區塊、胤天 9 個區塊；胤天版確認不含「吉爾塔斯」與「愛神」字樣。
- 七夕活動貼文兩站各 3 個表格、7 列武器能力。

## 2026-08-08 - 新增左右肩甲、黃道十二宮、支配之塔與父親節活動
...
### 異動
- 新增 `pages/armor-shoulder-guard.html`（左右肩甲介紹）：肩甲強化卷軸機率表（+1 50% ～ +10 1%）、左肩甲四職業（戰士／弓手／法師／坦克）與右肩甲四路線（血量／防禦／魔防／戰鬥）各 +0～+10 能力表，並標註安定值 0、強化失敗消失。
- 新增 `pages/features-zodiac.html`（黃道12宮介紹）：十二宮石板能力、四大星象能力、黃道十二宮最終能力，以及 NPC「道具製作・黃道使者」的完整合成材料表。
- 新增 `pages/features-domination-tower.html`（支配之塔）：進入方式（傲慢之塔王層尋找支配之塔傳送師）、十種 BOSS 靈魂基礎能力、靈魂昇華進化至 9 階、小怪掉落兌換隨機支配靈魂箱與萬物收購商 7,500 靈魂保底機制。
- `assets/css/style.css` 新增三個頁面的樣式區塊（肩甲 hero／左右分區色系／各表格配色、黃道 hero／石板與合成表、支配之塔 hero／靈魂表／昇華卡片）。
- `assets/js/forum.js` 桌機與手機選單新增「🎖️ 左右肩甲介紹」（防具道具區）、「🗼 支配之塔」與「♈ 黃道12宮介紹」（遊戲特色區）。
- `pages/armor-overview.html` 新增左右肩甲卡片並更新導覽敘述；`pages/features-overview.html` 新增支配之塔與黃道12宮卡片。
- `data/posts.json` 新增活動公告「🎁 父親節活動 ── 爸爸的私房錢」與「💎 父親節活動 ── 88 父親節・付清節的禮包」（皆為 2026/08/09 ~ 2026/09/09，狀態：進行中、置頂）。
- `data/posts.json` 新增「0809-更新歷程」（狀態：預計更新），涵蓋奇岩武防商人販售等級獎勵裝備、遺忘之島與傲慢之塔全區瞬移、左右肩甲系列、遊戲輔助 BOSS 表、黃道使者 NPC、支配之塔，以及開服活動回收提醒，並內嵌三個新頁面的連結。
- `data/posts.json` 將「🎉 開服活動 ── 歡慶開服慶」與「💎 開服活動 ── 開服贊助好禮」狀態改為已結束並取消置頂。

### 驗證
- `node scripts/build-search-index.js` 重建 `data/search-index.json`，共 67 筆，含三個新頁面。
- `node` 解析 `data/posts.json` 通過，共 17 筆；兩篇開服活動狀態已為「已結束」。
- 三個新頁面與兩個總覽頁的 div／section／table／tr／a 標籤數量對稱；`assets/js/forum.js`、`assets/js/search.js` 語法檢查通過。
...
- 內容涵蓋：龍印魔石上架官方介紹、強化卷軸現階段僅能透過推文幣兌換（與新手引導講法一致）、藍鑽商城放入龍印魔石販售；墮落 BOSS 新增墮落斗篷／長袍／手套／長靴；古老的巨劍與古老的劍改為可強化（安定 6）；傳送師 NPC 預計 8 月前更新完畢。
...
### 異動
- `data/posts.json` 新增 `07/23~26-更新歷程`（狀態：已更新），美編分為「伺服器合併／平衡性調整／掉落與商城／問題修復」四大區塊，並以綠框（已完成）與紅框（伺服器合併）強調重點。
- 內容涵蓋：太陽神 × 愛神伺服器合併、夜月之力掉落全面提高、雜貨商人新增「練功地圖（限 LV.55 以下進入）」、熾炎天使弓傷害提高並加入風屬性攻擊、法師對怪物傷害係數調整、冰魔出生速度修正，以及傲慢之塔怪物任務計算、奇古獸／鎖鏈劍武器祝福、生命樹果加速藥水圖示等問題修復。

### 驗證
- `node` 解析 `data/posts.json` 通過，新貼文為陣列首筆、共 13 筆。
- `data/search-index.json` 未變動（建置腳本不索引 posts，毋須重建）。
